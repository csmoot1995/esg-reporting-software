"""
Alert engine: carbon intensity spikes, water inefficiency, energy anomalies, sensor drift.
...
CARBON_PER_WORKLOAD_HOUR_MAX = _float_env("TELEMETRY_ALERT_CARBON_PER_WORKLOAD_HOUR_MAX", 5.0)
WUE_MAX_L_PER_KWH = _float_env("TELEMETRY_ALERT_WUE_MAX", 2.0)
WATER_PER_WORKLOAD_HOUR_MAX = _float_env("TELEMETRY_ALERT_WATER_PER_WORKLOAD_HOUR_MAX", 50.0)
RECLAIMED_WATER_MIN_PCT = _float_env("TELEMETRY_ALERT_RECLAIMED_WATER_MIN_PCT", 20.0)
ENERGY_EFFICIENCY_MAX = _float_env("TELEMETRY_ALERT_ENERGY_EFFICIENCY_MAX", 2.0)
COOLING_ENERGY_PCT_MAX = _float_env("TELEMETRY_ALERT_COOLING_ENERGY_PCT_MAX", 50.0)
COP_MIN = _float_env("TELEMETRY_ALERT_COP_MIN", 2.0)
...
def evaluate_carbon_intensity(grid_kg_per_kwh: float | None, carbon_per_workload_hour: float | None) -> list[dict[str, Any]]:
    alerts = []
    ...
    if carbon_per_workload_hour is not None and carbon_per_workload_hour > CARBON_PER_WORKLOAD_HOUR_MAX:
        alerts.append({
            "metric": "carbon_per_workload_hour",
            "value": carbon_per_workload_hour,
            "threshold": CARBON_PER_WORKLOAD_HOUR_MAX,
            "severity": "CRITICAL",
            "timestamp": _ts(),
        })
    return alerts
...
def evaluate_water(wue_l_per_kwh: float | None, water_per_workload_hour: float | None, reclaimed_pct: float | None) -> list[dict[str, Any]]:
    alerts = []
    ...
    if water_per_workload_hour is not None and water_per_workload_hour > WATER_PER_WORKLOAD_HOUR_MAX:
        alerts.append({
            "metric": "water_per_workload_hour",
            "value": water_per_workload_hour,
            "threshold": WATER_PER_WORKLOAD_HOUR_MAX,
            "severity": "WARNING",
            "timestamp": _ts(),
        })
...
def evaluate_cooling(efficiency_ratio: float | None, cooling_energy_pct: float | None, cop_value: float | None) -> list[dict[str, Any]]:
    alerts = []
    if efficiency_ratio is not None and efficiency_ratio > ENERGY_EFFICIENCY_MAX:
        alerts.append({
            "metric": "energy_efficiency_ratio",
            "value": efficiency_ratio,
            "threshold": ENERGY_EFFICIENCY_MAX,
            "severity": "WARNING",
            "timestamp": _ts(),
        })
    ...
    if cop_value is not None and cop_value < COP_MIN and cop_value > 0:
        alerts.append({
            "metric": "cop",
            "value": cop_value,
            "threshold_min": COP_MIN,
            "severity": "WARNING",
            "timestamp": _ts(),
        })
    return alerts
