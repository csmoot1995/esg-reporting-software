...
from .storage import payload_hash, normalize_timestamp, insert_raw, insert_carbon, insert_water, insert_efficiency, insert_hardware, insert_data_quality
...
from .units import normalize_energy, normalize_water, seconds_to_gpu_hours
...
def _run_calculations_and_store(
    ...
    if p.energy:
        ...
        if facility_kwh is None and it_kwh is not None:
            facility_kwh = it_kwh
    ...
    if total_kg > 0:
        insert_carbon("total_kg_co2e", total_kg, "kg_co2e", p.asset_id, p.region, "total", ef_ver, observation_utc, raw_id, lineage_dict)
    if scope1 > 0:
        insert_carbon("scope1_kg_co2e", scope1, "kg_co2e", p.asset_id, p.region, "scope1", ef_ver, observation_utc, raw_id, lineage_dict)
    if scope2 > 0:
        insert_carbon("scope2_kg_co2e", scope2, "kg_co2e", p.asset_id, p.region, "scope2_location", ef_ver, observation_utc, raw_id, lineage_dict)
    ...
    if p.compute:
        if p.compute.gpu_hours is not None:
            gpu_hours = p.compute.gpu_hours
        elif p.compute.run_duration_seconds is not None and p.compute.gpu_count is not None:
            gpu_hours = seconds_to_gpu_hours(p.compute.run_duration_seconds, p.compute.gpu_count or 1.0)
        training_runs = p.compute.training_runs or 0
        inference_requests = p.compute.inference_requests or 0

    workload_value = p.workload_value
    workload_unit = p.workload_unit
    if (workload_value is None or workload_unit is None) and gpu_hours is not None:
        workload_value = gpu_hours
    ...
    if workload_value is not None and workload_value > 0 and total_kg and total_kg > 0 and workload_unit:
        ...
        insert_carbon("carbon_intensity", intensity_ratio, intensity_unit, p.asset_id, p.region, None, ef_ver, observation_utc, raw_id, lineage_dict)

    if gpu_hours and gpu_hours > 0 and total_kg and total_kg > 0:
        cpgh = calc_carbon.carbon_per_gpu_hour(total_kg, gpu_hours)
        insert_carbon("carbon_per_gpu_hour", cpgh, "kg_co2e_per_gpu_hour", p.asset_id, p.region, None, ef_ver, observation_utc, raw_id, lineage_dict)
        alert_list.extend(alerts.evaluate_carbon_intensity(None, cpgh))
    if training_runs and training_runs > 0 and total_kg and total_kg > 0:
        cptr = calc_carbon.carbon_per_training_run(total_kg, training_runs)
        insert_carbon("carbon_per_training_run", cptr, "kg_co2e_per_run", p.asset_id, p.region, None, ef_ver, observation_utc, raw_id, lineage_dict)
    if inference_requests and inference_requests > 0 and total_kg and total_kg > 0:
        cpir = calc_carbon.carbon_per_inference_request(total_kg, inference_requests)
        insert_carbon("carbon_per_inference_request", cpir, "kg_co2e_per_request", p.asset_id, p.region, None, ef_ver, observation_utc, raw_id, lineage_dict)

    # Grid intensity from payload or factors
    if p.carbon and p.carbon.grid_carbon_intensity_kg_per_kwh is not None:
        insert_carbon("grid_carbon_intensity", p.carbon.grid_carbon_intensity_kg_per_kwh, "kg_co2e_per_kwh", p.asset_id, p.region, "grid", ef_version, observation_utc, raw_id, lineage_dict)
    ...
    if p.water:
        ...
        if tw > 0:
            insert_water("total_withdrawal_liters", tw, "liters", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        ...
        if withdrawal and it_kwh and it_kwh > 0:
            ...
            insert_water("wue", wue_val, "L_per_kWh", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
            alert_list.extend(alerts.evaluate_water(wue_val, None, rec_pct))
        if gpu_hours and gpu_hours > 0 and tw > 0:
            wpgh = calc_water.water_per_gpu_hour(tw, gpu_hours)
            insert_water("water_per_gpu_hour", wpgh, "liters_per_gpu_hour", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
            alert_list.extend(alerts.evaluate_water(None, wpgh, rec_pct))
        if training_runs and training_runs > 0 and tw > 0:
            wptr = calc_water.water_per_training_run(tw, training_runs)
            insert_water("water_per_training_run", wptr, "liters_per_run", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        if rec_pct >= 0:
            insert_water("reclaimed_water_pct", rec_pct, "pct", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
    ...
    if facility_kwh is not None and it_kwh is not None and it_kwh > 0:
        ...
        insert_efficiency("pue", pue_val, "ratio", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        insert_efficiency("dcie", dcie_val, "ratio", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        ...
        if cooling_kwh is not None:
            ...
            insert_efficiency("cooling_energy_pct", cool_pct, "pct", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
            alert_list.extend(alerts.evaluate_cooling(None, cool_pct, None))
        if gpu_hours and gpu_hours > 0:
            epg = calc_efficiency.energy_per_gpu_hour(it_kwh, gpu_hours)
            insert_efficiency("energy_per_gpu_hour", epg, "kWh_per_gpu_hour", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        if training_runs and training_runs > 0:
            eptr = calc_efficiency.energy_per_training_run(it_kwh, training_runs)
            insert_efficiency("energy_per_training_run", eptr, "kWh_per_run", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
    ...
    if p.hardware:
        ...
        if util >= 0:
            insert_hardware("utilization_pct", util, "pct", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        if idle >= 0:
            insert_hardware("idle_rate_pct", idle, "pct", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)

    # Data quality
    if p.data_quality:
        dq = p.data_quality
        conf = calc_dq.confidence_score(dq.completeness_pct, dq.latency_seconds, dq.outlier_flag, dq.drift_flag)
        insert_data_quality("confidence_score", conf, "0-1", p.asset_id, p.region, observation_utc, raw_id, lineage_dict)
        if dq.drift_flag:
    ...
    summary = {
        ...
        "carbon_per_gpu_hour": calc_carbon.carbon_per_gpu_hour(total_kg, gpu_hours) if gpu_hours and gpu_hours > 0 else None,
...
def ingest():
    ...
    if include_scorecard:
        out["scorecard"] = scorecard.sustainability_score(
            carbon_per_gpu_hour=summary.get("carbon_per_gpu_hour"),
            water_per_gpu_hour=None,
            pue=summary.get("pue"),
...
def metrics_report():
    ...
    try:
        ...
        report = {"carbon": [], "water": [], "efficiency": [], "hardware": [], "data_quality": []}
        for table, key in [
